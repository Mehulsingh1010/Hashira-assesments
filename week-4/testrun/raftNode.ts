...
// Define the state as a union type of strings
type NodeState = "Follower" | "Candidate" | "Leader";

// Use an object to hold the values, if needed for referencing strings
const NodeStates = {
    Follower: "Follower" as NodeState,
    Candidate: "Candidate" as NodeState,
    Leader: "Leader" as NodeState,
};
// Then, you'd use NodeStates.Follower instead of NodeState.Follower throughout your code.
...
class RaftNode {
  ...
  private state: NodeState = NodeStates.Follower;
  ...
  public async handle_set(request: ClientRequest): Promise<ClientResponse> {
    if (this.state !== NodeStates.Leader) {
  ...
  public async handle_request_vote(
    ...
    if (request.term > this.current_term) {
      ...
      this.voted_for = null;
      this.state = NodeStates.Follower;
    }
  ...
  private async start_election(): Promise<void> {
    ...
    this.state = NodeStates.Candidate;
    this.current_term += 1;
    ...
    if (this.state === NodeStates.Candidate && votes >= this.majority()) {
      this.become_leader();
    } else if (this.state === NodeStates.Candidate) {
  ...
  private async leader_heartbeat(): Promise<void> {
    while (this.state === NodeStates.Leader) {
  ...
  private async send_append_entries_to_all(): Promise<void> {
    if (this.state !== NodeStates.Leader) return;
  ...
  public get_status(): NodeStatus {
    return {
      ...
      is_leader: this.state === NodeStates.Leader,
...
const all_peers: Peer[] = [
    ...
    { id: 4, address: "http://192.168.138.252:7004" }, // Srishti
  // Ajas
    { id: 5, address: "http://192.168.137.144:7005" }, 
    // { id: 6, address: "http://192.168.137.145:7006" },
];
